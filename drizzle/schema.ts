...
/**
 * Receipts table - comprehensive receipt storage with OCR data
 */
export const receipts = mysqlTable("receipts", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  transactionId: int("transactionId"), // Optional link to budget_transactions
  sharedBudgetTransactionId: int("sharedBudgetTransactionId"), // Optional link to shared_budget_transactions
  
  // Receipt Metadata
  merchantName: varchar("merchantName", { length: 255 }).notNull(),
  merchantAddress: text("merchantAddress"),
  merchantPhone: varchar("merchantPhone", { length: 50 }),
  receiptDate: timestamp("receiptDate").notNull(),
  receiptNumber: varchar("receiptNumber", { length: 100 }),
  paymentMethod: mysqlEnum("paymentMethod", ["cash", "credit_card", "debit_card", "mobile_payment", "other"]),
  ...
  // OCR Processing
  ocrStatus: mysqlEnum("ocrStatus", ["pending", "processing", "completed", "failed"]).default("pending"),
  ocrProvider: varchar("ocrProvider", { length: 50 }),
  ocrRawText: text("ocrRawText"),
  ocrConfidence: int("ocrConfidence"), // 0-100
  ocrProcessedAt: timestamp("ocrProcessedAt"),
  ocrErrorMessage: text("ocrErrorMessage"),
  
  // Categorization
  categoryId: int("categoryId"),
  suggestedCategory: varchar("suggestedCategory", { length: 100 }),
  categoryConfidence: int("categoryConfidence"), // 0-100
  
  // Status & Metadata
  status: mysqlEnum("status", ["draft", "verified", "disputed", "archived"]).default("draft"),
  notes: text("notes"),
  tags: text("tags"), // JSON array
  isReimbursable: int("isReimbursable").default(0),
  reimbursementStatus: mysqlEnum("reimbursementStatus", ["pending", "approved", "rejected", "paid"]),
...
/**
 * Receipt Line Items table - individual items from receipts
 */
export const receiptLineItems = mysqlTable("receipt_line_items", {
  ...
  quantity: int("quantity").default(1000).notNull(), // stored as quantity * 1000 (3 decimal places)
  ...
  // Metadata
  lineNumber: int("lineNumber"), // order in receipt
  isRefunded: int("isRefunded").default(0),
  notes: text("notes"),
