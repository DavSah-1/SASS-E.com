-- ============================================================================
-- Supabase Row Level Security (RLS) Policies - AUTO-GENERATED
-- Generated from actual table structures
-- ============================================================================
...
CREATE POLICY "Users can view their own translate_messages" ON translate_messages FOR SELECT USING (sender_id = auth.uid()::text);
CREATE POLICY "Users can create their own translate_messages" ON translate_messages FOR INSERT WITH CHECK (sender_id = auth.uid()::text);
CREATE POLICY "Users can update their own translate_messages" ON translate_messages FOR UPDATE USING (sender_id = auth.uid()::text);
CREATE POLICY "Users can delete their own translate_messages" ON translate_messages FOR DELETE USING (sender_id = auth.uid()::text);
...
-- ============================================================================
-- COMPLEX RLS POLICIES (Tables requiring joins through parent tables)
-- ============================================================================
...
CREATE POLICY "Creators can add shared budget members" ON shared_budget_members FOR INSERT WITH CHECK (
  shared_budget_id IN (SELECT id FROM shared_budgets WHERE created_by = auth.uid()::text)
);
CREATE POLICY "Creators can update shared budget members" ON shared_budget_members FOR UPDATE USING (
  shared_budget_id IN (SELECT id FROM shared_budgets WHERE created_by = auth.uid()::text)
);
CREATE POLICY "Creators and members can delete shared budget members" ON shared_budget_members FOR DELETE USING (
...
CREATE POLICY "Creators and editors can create shared budget transactions" ON shared_budget_transactions FOR INSERT WITH CHECK (
  shared_budget_id IN (SELECT id FROM shared_budgets WHERE created_by = auth.uid()::text OR id IN (SELECT shared_budget_id FROM shared_budget_members WHERE user_id = auth.uid()::text AND role IN ('owner', 'editor')))
);
CREATE POLICY "Creators and editors can update shared budget transactions" ON shared_budget_transactions FOR UPDATE USING (
  shared_budget_id IN (SELECT id FROM shared_budgets WHERE created_by = auth.uid()::text OR id IN (SELECT shared_budget_id FROM shared_budget_members WHERE user_id = auth.uid()::text AND role IN ('owner', 'editor')))
);
CREATE POLICY "Creators and editors can delete shared budget transactions" ON shared_budget_transactions FOR DELETE USING (
  shared_budget_id IN (SELECT id FROM shared_budgets WHERE created_by = auth.uid()::text OR id IN (SELECT shared_budget_id FROM shared_budget_members WHERE user_id = auth.uid()::text AND role IN ('owner', 'editor')))
);
...
CREATE POLICY "Users can view message translations in their conversations" ON translate_message_translations FOR SELECT USING (
  message_id IN (SELECT id FROM translate_messages WHERE conversation_id IN (SELECT id FROM translate_conversations WHERE creator_id = auth.uid()::text OR id IN (SELECT conversation_id FROM translate_conversation_participants WHERE user_id = auth.uid()::text)))
);
